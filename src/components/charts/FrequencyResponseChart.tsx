 ...
 *
 * Bode magnitude plot showing frequency response (log-log).
 * Displays system gain vs frequency from 20Hz to 50kHz.
 */
...
const DEFAULT_Y_RANGE: [number, number] = [-60, 20]

export const FrequencyResponseChart: React.FC = () => {
    ...
    const traces: Data[] = [
      {
        ...
        y: response.map((p) => p.magnitudeDb),
        type: 'scatter',
        mode: 'lines',
        name: transformer.enabled ? 'With Transformer' : 'Response',
  ...
  return (
    <ChartWrapper
      ...
      explanation={CHART_EXPLANATIONS.frequencyResponse}
    >
      <Plot
        data={data}
        layout={layout}
